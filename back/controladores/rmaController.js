...
const gestionarRma = {
  ...
  deleteRma: async (req, res) => {
    ...
    try {
      ...
      if (result.affectedRows > 0) {
        res.status(200).json({ success: true, message: 'RMA eliminado correctamente.' });
      } else {
        res.status(404).json({ success: false, message: 'RMA no encontrado.' });
      }
    } catch (error) {
      console.error('Error al eliminar RMA:', error);
      res.status(500).json({ success: false, message: 'Error al eliminar RMA.' });
    } finally {
      if (connection) {
        connection.release();
      }
...
getListarProductosRma: async (req, res) => { 
  ...
  try {
      ...
      setTimeout(async () => {
        ...
        res.json(productos);
      }, 10000); // Simula un retraso de 3 segundos
